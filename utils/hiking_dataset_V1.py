...
class HikingDataset(Dataset):
    ...
    def _create_features(self):
        ...
        ## 构造特征工程：晚间最大值和均值
        construct_feature = []
        construct_feature.append(self.elec_feature.values.reshape(-1,96,len(elec_feature_columns))[:,64:].max(axis=1)  )
        construct_feature.append(self.elec_feature.values.reshape(-1,96,len(elec_feature_columns))[:,64:].mean(axis=1)  )
        self.elec_feature = np.concatenate(construct_feature, axis=1)
        
        self.mask_index = np.ones(self.elec_feature.shape[1],dtype=bool)
        self.mask_index[[0,1,5,6]] = False  # 预测日的价格和负荷率无法获得
        
        self.features = self.elec_feature
    ...
    def _create_sequences(self):
        ...
        for i in range(len(label_values)):
            ...
            target_feature = self.features[i,self.mask_index]
    ...
    def _enhance_samples(self, noise_std=0.01, time_shift_range=1):
        ...
        for idx in positive_indices:
            ...
            if np.random.random() < 0.5:
                ...
                if shift != 0:
                    # 对序列特征进行时间偏移
                    shifted_seq_feature = np.roll(seq_feature, shift, axis=1) #### axis=1 是“特征维”，不是“时间维”，是否需要改成axis=0？
...
if __name__ == "__main__":
    ...
    dataset = HikingDataset(
        feature_data_path="/data3/wangjingwei/intern-hr/electricity-price-prediction-hiking_predict/data/processed/shanxi_new.parquet",
        label_data_path="/data3/wangjingwei/intern-hr/electricity-price-prediction-hiking_predict/data/processed/hiking_01_dataset.csv",
        sequence_length=7,
